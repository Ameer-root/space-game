...
if(frame % 500 === 0)
{
    ...
    grids.forEach((grid,gridIndex) => {
        grid.update()
            ...
            for(let j = bombs.length -1; j>= 0; j--){
                ...
                //if bomb touches invader, remove invaders
        ...
        if(rectangularCollision({rectangle1: invader, rectangle2: player}) && !game.over) endGame()
    ...
    if(keys.space.pressed && player.powerUp === 'MachineGun' && frame % 2 === 0 && !game.over){
       
   if(frame % 6 === 0) audio.shoot.play()
...
switch(key){
    ...
    case ' ':
        ...
        if(player.powerUp === 'MachineGyn') return
